...
def poll(msg, chat_id, chat_type, from_id):
	...
	if chat_type == 'group' or chat_type == 'supergroup':
		try:
			message_with_inline_keyboard = bot.sendMessage(chat_id, poll_of_the_day, reply_markup=markup) if (from_id == 66441008) or (from_id == 163329729) else bot.sendMessage(chat_id, 'Solo il Gran Maestro del Concilio puÃ² indire un sondaggio')
...
def exitpoll(msg, chat_id, from_id):
	if (from_id != 66441008) and (from_id != 163329729):
		bot.sendMessage(chat_id, 'Solo il Gran Maestro può chiudere un sondaggio in corso')
		if from_id == 146874789:
			risp = ['Bea piantala', 'Ti ho detto che solo il Gran Maestro può ordinarlo', 'Non sei un Gran Maestro.', 'Che scassamaroni', 'testarda eh', 'Solo il Gran Maestro!', 'Non ti obbedirò mai']
    ...
    try:
        if from_id not in votanti.keys():
            ...
            if votanti[from_id] == data:
                bot.answerCallbackQuery(query_id, text=msg['from']['username'] + ' ha già espresso il suo voto')
            else:
                risultati[data] += 1
